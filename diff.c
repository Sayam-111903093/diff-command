...
void diff(list_lcs l, list l1, list l2){
	node_lcs *ptr = l.rear;
	int k1 = length(l1), k2 = length(l2);
	do{
		if(ptr == NULL){  //tells us that nothing is common
			if(k1 == 1 && k2 == 1)
				printf("1c1\n");
			else if(k1 == 1 && k2 != 1)
				printf("1c1,%d\n", k2);
		...
		if(ptr == l.rear){  //only one thing is common
		...
		if(ptr == l.front){  //checking from backwards
...
void _cdiff(list_lcs l, list l1, list l2){  //context mode
	int k1 = length(l1), k2 = length(l2), k3 = length_lcs(l);
...
void _ydiff(list_lcs l, list l1, list l2){   // y mode
	int k1 = length(l1), k2 = length(l2), k3,  flag1 = 0, flag2 = 0;  //flags indicate that their is difference in files
...
void _udiff(list_lcs l, list l1, list l2){  // unified mode
	...
	if(l.rear == NULL){  //nothing is common in both files
		do{
			printf("-");
			printline(ptr->data);
			ptr = ptr->next;
		...
		if(ptr != l1.rear){ //prints uncommon in file 1
			while(ptr != l1.rear){
				printf("-");
				printline(ptr->data);
				ptr = ptr->next;
		...
		if(qtr != l2.rear){  // prints uncommon in file 2
			while(qtr != l2.rear){
				printf("+");
				printline(qtr->data);
				qtr = qtr->next;
